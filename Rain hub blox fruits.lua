...
MainTab:AddSection({"Auto Level"})

local playerName = game.Players.LocalPlayer.Name
MainTab:AddParagraph({"Bem-vindo(a) " .. playerName .. "!", ""})
...
elseif game.PlaceId == 7449423635 then
    ...
    Plr:Kick("Do not Support, Please wait...")
end
...
function CheckQuest() 
    ...
    if World1 then
        ...
        elseif MyLevel >= 375 and MyLevel <= 399 then
            ...
            if _G.AutoFarm and (CFrameQuest.Position - HRP.Position).Magnitude > 10000 then
...
function TweenTo(cf)
    if HRP and cf then
        ...
        local info = TweenInfo.new(math.max(time, 0.5), Enum.EasingStyle.Linear)
...
function AcceptQuest()
    local args = {"StartQuest", NameQuest, LevelQuest}
    local success, result = pcall(function()
        return game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
    end)
    if not success then
        warn("Falha ao aceitar a quest: " .. tostring(result))
    end
    wait(0.5)
end
...
function AutoLevel()
    while _G.AutoFarm and task.wait(0.1) do
        pcall(function()
            CheckQuest()
            if not NameMon or not CFrameQuest or not CFrameMon then
                warn("Quest ou monstro não configurado corretamente!")
                return
            end
            if not Plr.PlayerGui.Main.Quest.Visible then
                ...
                local enemy = CheckMon()
                if enemy then
                    repeat
                        TweenTo(enemy.HumanoidRootPart.CFrame * CFrame.new(0, 15, 0))
                        AutoAttack(enemy)
                        wait(0.1)
...
MainTab:AddButton({
    Name = "Debug Info",
    Callback = function()
        print("Level: ", CheckLevel())
        print("Quest: ", NameQuest, "LevelQuest: ", LevelQuest)
        print("Monstro: ", NameMon)
        print("CFrameQuest: ", CFrameQuest)
        print("CFrameMon: ", CFrameMon)
        print("Quest Ativa: ", Plr.PlayerGui.Main.Quest.Visible and "Sim" or "Não")
